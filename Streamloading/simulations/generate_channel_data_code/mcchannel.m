...
function c2=mcchannel(c1,TPD,rtype,ltype,granu,range)
...
for i=indstart:indend-1
    for j=1:granu
        ChannelPDF=[ChannelPDF,[(TPD(1,i+1)-TPD(1,i))/granu*j+TPD(1,i);(TPD(ind,i+1)-TPD(ind,i))/granu]];
    end
end
...
transprob(indcur)=1-sum(transprob);
