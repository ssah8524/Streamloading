...
function return_status_INDEX=runsim_sl_fullbase(fnarg)
...
user_present=ones(1,num_users);%   what does this represent?
beta_enh_bar=0*ones(1,num_users);
beta_base_bar = -0.1*ones(1,num_users);
...
base_size_index = 1;
enh_layer = 6;
b_base_init=20*ones(1,num_users);
b_enh_init=20*ones(1,num_users);
...
epsilon=0.05;
%   then, what is the variability importance?
eta_RVBAR =.01;
%   Is it in units of Mb per slot?
rmin_RVBAR=.001;

%   Let's try it first without data cost.
xi=.01;%   Unit cost for data transmission (p_i^d)
...
%base_queue=zeros(1,num_users);
%pending_base = zeros(1,num_users);

total_base = zeros(1,num_users);
base_queue = max_base_segments*ones(1,num_users);
...
attempted_enh = zeros(4500,num_users,2);

%files that ensure the correctness of the algorithms
out_of_time_dist = zeros(num_users,150000);
...
while min(total_base)<=min_seg_sim
    ...
    %buffer_occupancy = (total_base * t_seg) - floor(((slot-count_rebuff_slots) * t_slot));
    %buffer_occupancy(buffer_occupancy > max_base_segments)
    %if ~isempty(buffer_occupancy(buffer_occupancy >= max_base_segments))
    %    buffer_occupancy
    %end

    %periodically, new base layer segment become available to
    ...
    %flag is raised if there are base layers to download for any
    %user
    ...
    buffer_fill(:,slot) = (total_base * t_seg) - ((slot-count_rebuff_slots) * t_slot);
    ...
    elseif rate_allocation_scheme==0%RVBAR
        if slot<LPF_run_slots_before_using_RVBAR %  Use proportional fairness.
            'PF';
            rate_alloc=PF_long_run_rate_allocation(ch_gain(:,slot),base_queue);
        else
            'RVBAR';
    ...
    %check if requested enhancement layers have started downloading already
    if sum(base_queue) == 0
    ...
    if base_flag
        ...
        if(~isempty(finish_base_res))
            pending_base(finish_base_res) = pending_base(finish_base_res)+...
                ...
                total_base(finish_base_res)))';
        end
    else
        %Determine out of time arrived enhancement layers
        ...
        if ~isempty(front_enh) && slot > 3 * t_seg/t_slot
            for u = front_enh
                ...
                b_enh_est(u) = max(b_enh_est(u) - (ceil((slot - count_rebuff_slots(u))*t_slot/t_seg) - total_enh(u)),0);
            ...
            if quality_adaptation_scheme==0 %QVBAR
                while (flag == false)
                    ...
                    if total_enh(user_index)<segments_to_run_greedy_before_QVBAR
                        ...
                        var_imp_scaling=variability_importance_constant;
                        
                        %Enable the following lines in case we want to use the
                        %lipschitz values for virtual buffers.
...
dlmwrite('out_of_time_dist_sl.txt',out_of_time_dist(:,1:100000));
dlmwrite('buffer_fill_sl.txt',buffer_fill(:,1:90000));
dlmwrite('enh_fill_sl.txt',enh_fill(:,1:slot));
dlmwrite('virtual_buffer_sl.txt',virtual_b(:,1:slot));
...
if sum(base_queue) ~= 0
    ...
    fn_b=fn_b_scale(base_queue > 0).*max(b_base_est(base_queue > 0)-...
        fn_b_offset(base_queue > 0),0).*user_present(base_queue > 0);
...
function rate_allocation_PFL=PF_long_run_rate_allocation(ch_gain_cur_slot,base_queue)
...
if sum(base_queue) ~= 0
    rate_allocation_PFL(base_queue > 0) = rate_allocation_PFL_tmp;
    %size(rate_allocation_PFL)
else
