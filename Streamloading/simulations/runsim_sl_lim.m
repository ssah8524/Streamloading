...
function return_status_INDEX=runsim_sl_lim(fnarg)
...
pbar_val=fnarg(6);max_base_segments=fnarg(8);enh_lim=fnarg(11);
...
while min(total_base) <= min_seg_sim
    ...
    enh_occupancy = (total_enh * t_seg) - ((slot - count_rebuff_slots) * t_slot);

    base_dl = find(buffer_occupancy < max_base_segments);
    enh_dl = find(enh_occupancy < enh_lim);
    first_hit(first_hit==0 & enh_occupancy >= enh_lim) = b_enh_est(first_hit == 0 & enh_occupancy >= enh_lim);
    ...
    elseif rate_allocation_scheme == 0 %RVBAR
        if slot<LPF_run_slots_before_using_RVBAR %Use proportional fairness.
            rate_alloc = PF_long_run_rate_allocation(ch_gain(:,slot),base_dl,enh_dl);
        else
            rate_alloc = RVBAR(ch_gain(:,slot),base_dl,enh_dl);
        end
    ...
    %%E-Mode
        ...
        for user_index = finish_enh           
            ...
            %b_enh_est(user_index)=max(b_enh_est(user_index)-epsilon*t_seg,0);
            if enh_occupancy(user_index) < enh_lim - 1
                b_enh_est(user_index) = max(b_enh_est(user_index) - epsilon*t_seg,0);
                first_hit(user_index) = 0;
            elseif enh_occupancy(user_index) >= enh_lim - 1 && first_hit(user_index) ~= 0
                b_enh_est(user_index) = first_hit(user_index);
            end
                %b_enh_est(user_index) = max(b_enh_est(user_index) - epsilon * t_seg,0);
        end
    end
...
function rate_allocation_RVBAR=RVBAR(ch_gain_cur_slot,base_dl,enh_dl)
...
if ~isempty(base_dl)
    ...
    fn_b=fn_b_scale(enh_dl).*max(b_enh_est(enh_dl)-...
        fn_b_offset(enh_dl),0).*user_present(enh_dl);
    ch_gain_cur_slot = ch_gain_cur_slot(enh_dl);
    users_active = user_present(enh_dl);
end
...
if ~isempty(base_dl)
    ...
    rate_allocation_RVBAR(enh_dl) = rate_allocation_RVBAR_tmp;
end
...
function rate_allocation_PFL=PF_long_run_rate_allocation(ch_gain_cur_slot,base_dl,enh_dl)
...
if ~isempty(base_dl)
    ...
    rate_allocation_PFL(enh_dl) = rate_allocation_PFL_tmp;
end
