...
function return_status_INDEX=runsim(fnarg)

global num_users rmin_RVBAR m_est v_est b_est d_est rho_est sigma_est epsilon  fn_b_scale user_present...
    fn_b_offset count_rebuff_slots slot total_number_of_segments_downloaded_by_users...
    segment_duration slot_duration size_of_current_segment_being_downloaded enh_layer
...
variability_importance_constant=0.1;%important_constant_1. 

count_rebuff_slots=zeros(1,num_users);
user_present=ones(1,num_users);

%VBAR parameters
%The fn_b and fn_d are the values for the lypschitz function for b
%and d respectively.
epsilon=0.05;
beta_bar=-0.2*ones(1,num_users);%-(1/21)*ones(1,num_users);
fn_b_scale=0.2*ones(1,num_users);%0.01*ones(1,num_users); 
...
tmp=strcat(tmp,num2str(33),'_T=',num2str(150000),'_round=',num2str(fnarg(8)));
...
qual_ind = ones(num_users,min_num_of_segments_to_simulate_for_each_user);
finish_time = zeros(num_users,1);
...
while min(total_number_of_segments_downloaded_by_users)<=min_num_of_segments_to_simulate_for_each_user
    ...
    buffer_fill(:,slot) = (total_number_of_segments_downloaded_by_users * segment_duration)...
        - ((slot-count_rebuff_slots) * slot_duration);
    ...
    elseif rate_allocation_scheme==0%RVBAR
        ...
        epsilon_slot=max(.001,1/slot); %doesn't change performance
    ...
    %Amir: This whole while loop is only for those users who have
    ...
    while(isempty(set_of_users_who_finish_segment_transmission_in_current_slot)==0)
        ...
        %Determine the finishing time of the requested download
        if total_number_of_segments_downloaded_by_users(user_index) == min_num_of_segments_to_simulate_for_each_user
            finish_time(user_index) = slot;
        end
        ...
        if total_number_of_segments_downloaded_by_users(user_index)<initial_num_segments_to_be_downloaded_at_low_quality
            ...
            elseif quality_adaptation_scheme==0 %QVBAR
                if total_number_of_segments_downloaded_by_users(user_index)<segments_to_run_greedy_before_QVBAR
                    ...
                    quality_choice_for_this_user=q_vals_for_this_user_latest_segment(selected_representation_index);
                    segment_size_for_quality_choice=sigma_vals_for_this_user_latest_segment(selected_representation_index);
                    qual_ind(user_index,total_number_of_segments_downloaded_by_users(user_index) + 1) = selected_representation_index;
                end
            elseif quality_adaptation_scheme==1%greedy segment selection based on current estimate of throughput
                selected_representation_index=max(1,sum((sigma_vals_for_this_user_latest_segment<=(0.99)*rho_est(user_index))));
                quality_choice_for_this_user=q_vals_for_this_user_latest_segment(selected_representation_index);
                segment_size_for_quality_choice=sigma_vals_for_this_user_latest_segment(selected_representation_index);
            end
        end
...
tmp=[chao_metric;QoE;mean_quality-quality_change;mean_quality;quality_std;quality_change;rebuf_time_est;count_rebuff_slots*slot_duration;price_per_sec;finish_time'];
...
dlmwrite('virtual_buffer_nova.txt',virtual_b(:,1:90000));
dlmwrite('buffer_fill_nova.txt',buffer_fill(:,1:90000));
%dlmwrite('qual_nova.txt',qual_ind);


returnstatus_INDEX=1;

function rate_allocation_RVBAR=RVBAR(ch_gain_cur_slot)
global   rmin_RVBAR b_est fn_b_scale fn_b_offset user_present
