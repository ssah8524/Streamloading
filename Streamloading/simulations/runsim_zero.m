...
function return_status_INDEX=runsim_zero(fnarg)
...
close all;min_num_of_segments_to_simulate_for_each_user=600;num_users=fnarg(1);
...
LPF_run_slots_before_using_RVBAR=1000;%needed to ensure that there is no rebuffering in the beginning for the weakest users due to poor allocation
...
%VBAR parameters
beta_bar=0.1*ones(1,num_users);%-(1/21)*ones(1,num_users);
...
while min(total_number_of_segments_downloaded_by_users)<=min_num_of_segments_to_simulate_for_each_user
    ...
    b_est=b_est+epsilon*slot_duration;    
    ...
    while(isempty(set_of_users_who_finish_segment_transmission_in_current_slot)==0)
        ...
        b_est(user_index)=b_est(user_index)-epsilon*segment_duration;
